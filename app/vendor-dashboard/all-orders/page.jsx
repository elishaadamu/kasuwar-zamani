  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32 pt-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        ...
        <div className="flex bg-white border border-gray-100 p-2 rounded-2xl shadow-sm h-fit">
           <div className="flex items-center gap-3 px-6 py-2">
             <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 shrink-0">
               <FaReceipt />
             </div>
             <div>
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Active Queue</p>
               <p className="text-lg font-black text-gray-900">{orders.length} Orders</p>
             </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
           ...
           <div className="bg-indigo-900 rounded-[2rem] p-8 text-white relative overflow-hidden group">
              ...
              <h4 className="text-lg font-black tracking-tight mb-4 relative z-10">Revenue Support</h4>
              <p className="text-xs text-indigo-200 leading-relaxed mb-6 relative z-10">Having trouble with a transaction? Our merchant assistance is online.</p>
              <button 
                type="button" 
            ...
            <>
              {paginatedOrders.map((order) => {
                ...
                return (
                  <div 
                    key={order._id} 
                    className="group bg-white rounded-[2rem] border border-gray-100 p-6 md:p-8 hover:shadow-2xl hover:shadow-indigo-900/5 transition-all duration-500 cursor-pointer relative overflow-hidden"
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-8 justify-between items-start md:items-center">
                      <div className="flex gap-5 items-center">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${status.color} shadow-inner transition-transform group-hover:scale-110 duration-500`}>
                          <FaBoxOpen className="w-6 h-6" />
                        </div>
                        <div>
                          <div className="flex flex-wrap items-center gap-2 mb-1.5">
                            <h3 className="text-lg font-black text-gray-900 tracking-tight uppercase">#{order._id?.slice(-8)}</h3>
                          ...
                          <div className="flex items-center gap-3 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                             <span className="flex items-center gap-1.5 font-mono"><FaCalendarAlt className="text-gray-300" /> {new Date(order.createdAt).toLocaleDateString()}</span>
                      ...
                      <div className="w-full md:w-auto flex flex-col items-end gap-1 pt-4 md:pt-0 border-t md:border-t-0 border-gray-50">
                         <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.3em]">Expected Inflow</p>
                         <p className="text-2xl font-black text-indigo-600 tracking-tighter">₦{order.totalAmount?.toLocaleString()}</p>
                         <div className="flex items-center gap-2 text-indigo-600 font-black text-[9px] uppercase tracking-widest group-hover:translate-x-2 transition-transform">
                            Resolve Transaction <FaArrowRight />
                       </div>
                      </div>
                    </div>

                    <div className="mt-6 pt-6 border-t border-gray-50 flex items-center gap-3 relative z-10">
