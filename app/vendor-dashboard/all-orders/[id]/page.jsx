        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="bg-white rounded-[2.5rem] border border-gray-100 p-10 shadow-sm relative overflow-hidden">
             ...
             <div className="flex flex-col md:flex-row justify-between gap-6 mb-12 relative z-10">
               <div>
                 <span className="text-indigo-600 font-black text-[10px] uppercase tracking-[0.4em] mb-2 block">Merchant Fulfillment View</span>
                 <h1 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Manifest #{order._id.slice(-12)}</h1>
                 <p className="mt-2 text-gray-500 font-medium italic">Pipeline Entry: {new Date(order.createdAt).toLocaleString()}</p>
               </div>
                  ...
                  {order.products?.map((item, idx) => (
                    <div key={idx} className="py-6 flex gap-6 items-center group">
                      ...
                      <div className="flex-1">
                        <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1 font-mono">ID: {item.productId?._id?.slice(-6) || "N/A"}</p>
                        ...
                        <p className="text-xs font-bold text-gray-400 mt-1 uppercase tracking-widest">Inflow Qty: {item.quantity || item.quantity_bought || 1}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-black text-indigo-600 tracking-tighter">₦{(item.price || item.productId?.price || 0).toLocaleString()}</p>
                        <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mt-1">Vendor Revenue</p>
                      </div>
                    </div>
                  ))}
                </div>
             ...
             <div className="mt-12 pt-10 border-t-2 border-dashed border-gray-50 flex justify-between items-center relative z-10">
                <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Total Transaction Impact</p>
                <div className="text-right">
                  <p className="text-4xl font-black text-indigo-600 tracking-tighter">₦{order.totalAmount?.toLocaleString()}</p>
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mt-1">Funds Secured</p>
                </div>
             </div>
           </div>
        </div>
        ...
        <div className="lg:col-span-1 space-y-8">
           ...
           <div className="bg-white rounded-[2.5rem] border border-gray-100 p-8 shadow-sm">
             <h3 className="text-xs font-black text-gray-900 uppercase tracking-widest mb-8 flex items-center gap-3">
                <div className="w-8 h-8 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600"><FaMapMarkerAlt /></div>
                Logistics Destination
             </h3>
             <div className="space-y-6">
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Delivery Sector</p>
                  <p className="text-sm font-bold text-gray-600 leading-relaxed italic border-l-4 border-indigo-100 pl-4 py-2 bg-gray-50 rounded-r-xl">
                    "{order.deliveryAddress || "Recipient has not finalized address."}"
                  </p>
                </div>
                <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                   <div className="flex -space-x-2">
                      <div className="w-8 h-8 rounded-full bg-indigo-100 border-2 border-white flex items-center justify-center text-[10px] font-black text-indigo-600"><FaUser /></div>
                      <div className="w-8 h-8 rounded-full bg-blue-100 border-2 border-white flex items-center justify-center text-[10px] font-black text-blue-600">ID</div>
                   </div>
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Secure Client</p>
                </div>
             </div>
           </div>

           {/* Performance Stats */}
           <div className="bg-indigo-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-indigo-900/10 group">
              <h3 className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-8 flex items-center gap-3">
                <div className="w-8 h-8 bg-white/10 rounded-xl flex items-center justify-center text-white"><FaChartLine /></div>
                Market Impact
              </h3>
              <div className="space-y-4">
                 ...
                 <p className="text-[10px] text-indigo-300 leading-relaxed mt-4 italic">Confirming this order streamlines your merchant credibility metric by 2.4%.</p>
              </div>
           </div>
           ...
           <div className="flex flex-col gap-4">
              <button 
                ...
                className="w-full flex items-center justify-center gap-3 bg-white border border-gray-100 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-900 hover:bg-gray-50 transition-all font-mono"
              >
                <FaReceipt /> PRINT PACKAGING SLIP
              </button>
              <button 
                onClick={() => router.push("/vendor-dashboard/inbox-support")}
                className="w-full flex items-center justify-center gap-3 bg-indigo-600 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all"
              >
                 DISPATCH ASSISTANCE <FaArrowRight />
              </button>
           </div>
        </div>
      </div>
