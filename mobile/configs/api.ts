...
axios.interceptors.response.use(
  ...
  async (error) => {
    if (error.response && error.response.status === 401) {
      ...
      if (!isPinError) {
        // You may want to check current route, but in React Native we can just emit an event
        // and handle it in the root layout to avoid redirect loops
        await AsyncStorage.removeItem('user');
        
        // Dispatch a custom event to handle session expiration globally
...
const getBaseUrl = () => {
  ...
  // Using the production API from .env.local as the default to ensure connectivity
  return 'https://ecommerce-1-o2b3.onrender.com/api';
};
